...
function renderCircom() {
  pushd "$CIRCUIT_DIR"
  echo sed -i '' "s/Main([0-9]*)/Main($INPUT_SIZE)/" ${CIRCUIT_NAME}_test.circom
  sed -i '' "s/Main([0-9]*)/Main($INPUT_SIZE)/" ${CIRCUIT_NAME}_test.circom
  popd
}

function compile() {
  pushd "$CIRCUIT_DIR"
  echo circom ${CIRCUIT_NAME}_test.circom --r1cs --sym --wasm
  circom ${CIRCUIT_NAME}_test.circom --r1cs --sym --wasm
# witness generation by c++ is not supported on M1 arm64
#  cd ${CIRCUIT_NAME}_test_cpp
...
function setup() {
  ...
  echo "${TIME[@]}" "$SCRIPT_DIR"/trusted_setup.sh "${CIRCUIT_NAME}" "$TAU_RANK"
  "${TIME[@]}" "$SCRIPT_DIR"/trusted_setup.sh "${CIRCUIT_NAME}" "$TAU_RANK"
# snarkjs fflonk setup ${CIRCUIT_NAME}_test.r1cs ${TAU_FILE} ${CIRCUIT_NAME}_test_0000.zkey
...
# snarkjs zkey export verificationkey ${CIRCUIT_NAME}_test_0001.zkey verification_key.json
  prove_key_size=$(ls -lh "$CIRCUIT_DIR"/${CIRCUIT_NAME}_test_0001.zkey | awk '{print $5}')
...
function generateWtns() {
  pushd "$CIRCUIT_DIR"
#  witness generation by c++ is not supported on M1 arm64
#  echo "${TIME[@]}" ${CIRCUIT_NAME}_test_cpp/${CIRCUIT_NAME}_test input_${INPUT_SIZE}.json witness.wtns
#  "${TIME[@]}" ${CIRCUIT_NAME}_test_cpp/${CIRCUIT_NAME}_test input_${INPUT_SIZE}.json witness.wtns
  echo node ${CIRCUIT_NAME}_test_js/generate_witness.js ${CIRCUIT_NAME}_test_js/${CIRCUIT_NAME}_test.wasm input_${INPUT_SIZE}.json witness.wtns
  "${TIME[@]}" node ${CIRCUIT_NAME}_test_js/generate_witness.js ${CIRCUIT_NAME}_test_js/${CIRCUIT_NAME}_test.wasm input_${INPUT_SIZE}.json witness.wtns
  popd
}
...
function normalProve() {
  pushd "$CIRCUIT_DIR"
  avg_time 10 snarkjs fflonk prove ${CIRCUIT_NAME}_test_fflonk.zkey witness.wtns proof.json public.json
#  "${TIME[@]}" snarkjs fflonk prove ${CIRCUIT_NAME}_test_0001.zkey witness.wtns proof.json public.json
...
function rapidProve() {
  pushd "$CIRCUIT_DIR"
  avg_time 10 "$RAPID_SNARK_PROVER" ${CIRCUIT_NAME}_test_0001.zkey witness.wtns proof.json public.json
  proof_size=$(ls -lh proof.json | awk '{print $5}')
  echo "Proof size: $proof_size"
  popd
}
...
function verify() {
  pushd "$CIRCUIT_DIR"
  avg_time 10 snarkjs fflonk verify verification_key.json public.json proof.json
