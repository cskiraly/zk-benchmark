...
# snarkjs plonk setup ${CIRCUIT_NAME}_test.r1cs ${TAU_FILE} ${CIRCUIT_NAME}_test_0000.zkey
...
function normalProve() {
  pushd "$CIRCUIT_DIR"
  avg_time 10 snarkjs plonk prove ${CIRCUIT_NAME}_test_plonk.zkey witness.wtns proof.json public.json
#  "${TIME[@]}" snarkjs plonk prove ${CIRCUIT_NAME}_test_0001.zkey witness.wtns proof.json public.json
...
function verify() {
  pushd "$CIRCUIT_DIR"
  avg_time 10 snarkjs plonk verify verification_key.json public.json proof.json
